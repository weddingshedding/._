...
form.addEventListener('submit', (e) => {
  ...
  const specialRequirement = document.getElementById('specialRequirement')?.value.trim() || '';
  const text = `Hello Wedding Shedding,%0AName: ${encodeURIComponent(name)}%0APhone: ${encodeURIComponent(phone)}%0AEvent Start / Kab Se: ${encodeURIComponent(eventStart)}%0AEvent End / Kab Tak: ${encodeURIComponent(eventEnd)}%0AEvent Type: ${encodeURIComponent(eventType)}%0AVenue / City: ${encodeURIComponent(venue)}%0ADetails: ${encodeURIComponent(message)}%0AAny Special Requirement: ${encodeURIComponent(specialRequirement)}`;
